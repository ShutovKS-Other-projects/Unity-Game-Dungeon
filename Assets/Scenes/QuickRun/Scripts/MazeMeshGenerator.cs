...
public class MazeMeshGenerator
{
    ...
    public Mesh FromData(int[,] data)
    {
        ...
        /*Да, и чуть не забыли про последний параметр: RecalculateNormals() подготавливает сетку для освещения.*/
        maze.RecalculateNormals();

        return maze;
    }

    /* Остальные 3 параметра AddQuad () — это тот самый список вершин, UV и треугольников, к которому нужно добавить значения. 
     * Для справки, первая строка метода получает индекс для начала; По мере добавления четырех квадратов, их индекс будет расти.*/
